...
- Led frontend delivery for a complaint management workflow with evidence attachment, multi-state tracking, and real-time status feedback
...
- Designed and built an evidence health monitoring dashboard with real-time alerts for expired, expiring-soon, and broken-link evidence items
...
- Built EduSoft.NET REST integration for automated academic-grade-to-conduct-score conversion using a configurable conversion matrix
...
- Designed multi-tenant RBAC: School → Faculty → Department → Class org hierarchy with cascading permission inheritance
...
**AQ EduEAQ** — Accreditation Management System
- Architected a microservices decomposition across 7 functional subsystems (.NET 9 / C#), with service boundaries aligned to independent scalability requirements
...
- Implemented EduSoft.NET smart merge sync: conflict resolution strategy updates matched records while preserving locally-created data not present in the source system
...
- Containerized services with Docker for environment consistency across dev, staging, and production
